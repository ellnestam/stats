...
var stats = {
	...
	for (var i = 0; i < data.length; i++) {
		...
		var sorted = _.sortBy(resources, function(r) {
		    var name = r.name;
		    var num = name.replace('/pod/', '').replace('.mp3', '').replace('avsnitt', '');
		    return parseInt(num);
		});
		
		for (var j = 0; j < sorted.length; j++) {
		    var resource = sorted[j];
		    var num = resource.count;
		    var name = resource.name.replace('/pod/', '').replace('.mp3', '');
